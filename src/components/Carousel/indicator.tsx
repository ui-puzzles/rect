...
const Indicator: FC<CarouselIndicatorProps & InternalIndicatorProps> = (props) => {
  const {
    className,
    style,
    position = 'bottom',
  ...
  const isValidPosition = ['bottom', 'left', 'right', 'top', 'outer'].includes(position);
  ...
  const clickEvents = triggerType === 'click' ? {
    onClick: handleSlide
  } : {};
  const hoverEvents = triggerType === 'hover' ? {
  ...
  return (
    <div className={classNames} style={style} {...clickEvents} >
      {
        new Array(slidesLength).fill(0).map((_, i) => <span
          data-index={i}
          {...hoverEvents}
